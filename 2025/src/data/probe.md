...
通常来说，$\text{MeV}$ 量级的电子可以近似看作一个顶点, 它会迅速在某个位置耗尽所有动能并发出光子, 这是一个非齐次泊松过程, 该过程的典型的时间尺度大概为 $10 \,\text{ns}$ , 其对应训练集（更新前）中 `PETruth` 这个 dataset 的 `LightTime` ; 对于相同能量量级的正电子, 其会先和电子一样消耗能量并发出光子, 然后再发生正负电子湮灭, 产生两个方向相反的、能量为 $511 \,\text{keV}$ 的 $\gamma$ 光子, 由于 $\gamma$ 光子能量较高, 其可能与许多电子发生康普顿散射, 又可以看作一群顶点.
