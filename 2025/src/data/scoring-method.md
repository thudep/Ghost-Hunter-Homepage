...
3. 请尽量保持 git 仓库的整洁, 不要将[比赛数据集](https://ghfile.thudep.com:7200/)内的文件添加到远程 git 仓库中 (它们已经被 `.gitignore` 所忽略, 你只要不手动更改 `.gitignore` , 理论上是不会被上传的), 如果被发现 git 仓库中上传了比赛数据集文件, 可能会被警告甚至罚分; 你的模型参数文件可以上传, 不会遭到罚分.